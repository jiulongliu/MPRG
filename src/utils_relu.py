...
import mnist_estimators_relu as mnist_estimators
...
def get_estimator(hparams, model_type):
    if hparams.dataset == 'mnist':
        if model_type == 'vae':
            ...
            estimator = mnist_estimators.csgm_estimator(hparams)      
        elif model_type == 'prGAN':
            estimator = mnist_estimators.prGAN_estimator(hparams)             
        elif model_type == 'omp':
        ...
        if model_type == 'lasso-dct':
            ...
            estimator = celebA_estimators.prGAN_estimator(hparams)             
        ...
        for image in imgdict[model_type]:
            ...
            if alg_labels and inner_c == 0:
                ...
                if model_type == 'OneShot':
                    axes[outer_counter,inner_c].yaxis.set_label_text('  OneShot',fontsize=font_size)
                else:
...
def get_learning_rate(global_step, hparams):
    if hparams.decay_lr:
        return tf.compat.v1.train.exponential_decay(hparams.learning_rate,
                                          global_step,
                                          50,
                                          0.7,
                                          staircase=True)
...
def get_optimizer(learning_rate, hparams):
    ...
    if hparams.optimizer_type == 'momentum':
        return tf.compat.v1.train.MomentumOptimizer(learning_rate, hparams.momentum)
        ...
        return tf.compat.v1.train.AdagradOptimizer(learning_rate)
    else:
...
def solve_lasso(A_val, y_val, hparams):
    if hparams.lasso_solver == 'sklearn':
        lasso_est = Lasso(alpha=hparams.lmbd,max_iter=1)#normalize=True
...
def get_opt_reinit_op(opt, var_list, global_step):
    ...
    opt_reinit_op = tf.compat.v1.variables_initializer(all_opt_variables)
