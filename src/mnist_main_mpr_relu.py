...
def main(hparams):
    ...
    for n_measurement in hparams.num_outer_measurement_ls:
        ...
        x_batch = loadmat('xs_generatedfromNN.mat')['xs']
        ic = 0
        for key, x in xs_dict.items():
            # print(key)
            x_batch_dict[key] = x_batch[ic] #placing images in dictionary
            xs_dict[key] = x_batch[ic]
            ic =  ic +1
        ...
        for ri in range(hparams.num_experiments):
            ...
            for ni in range(len(hparams.noise_std_ls)):
                ...
                if hparams.nonlinear_model == 'abs(Ax)':
                    y_batch_outer = np.abs(np.matmul(x_batch, A_outer))                    
                elif hparams.nonlinear_model == 'abs(Ax+eta)':
                    y_batch_outer = np.abs(np.matmul(x_batch/LA.norm(x_batch, axis=(1),keepdims=True), A_outer)+noise_batch)                 
                elif hparams.nonlinear_model == 'abs(Ax)+5sin(abs(Ax))+eta':
                    Ax_abs = np.abs(np.matmul(x_batch/LA.norm(x_batch, axis=(1),keepdims=True), A_outer))
                    y_batch_outer = Ax_abs+5.0*np.sin(Ax_abs) + noise_batch
                elif hparams.nonlinear_model == 'abs(Ax+eta)':
                    y_batch_outer = np.abs(np.matmul(x_batch/LA.norm(x_batch, axis=(1),keepdims=True), A_outer)+noise_batch)   
                elif hparams.nonlinear_model == 'abs(Ax)^2+eta':
                    y_batch_outer = np.abs(np.matmul(x_batch/LA.norm(x_batch, axis=(1),keepdims=True), A_outer)**2)+noise_batch  
                    ...
                    print(hparams.nonlinear_model+'is not in [abs(Ax)+eta,abs(Ax)+5sin(abs(Ax))+eta,abs(Ax+eta)]!')
                    break
                ...
                for k in range(hparams.max_outer_iter_step1):            
                ...
                # print('nv_tilde_batch0[4]:',nv_tilde_batch0[4],'nv_tilde_batch[4]',nv_tilde_batch[4],'y_tilde_batch0_sum',y_tilde_batch0.sum(),'y_tilde_batch_sum',y_tilde_batch.sum())                
            
                # print('Method list',hparams.method_ls)
                x_hats_dict = {}
                y_dict = {}                
                for method in hparams.method_ls:
                    # if method not in  ['MPRG_Step1','MPRG_Step2','MPRG','MPRGa','MPRS','APPGD']:
                    #     print(method+' is not in [MPRG_Step1,MPRG_Step2,MPRG,MPRS,APPGD]!')
                    #     break
                    x_hats_dict[method] = {}     
                    ...
                    if method == 'prGAN':
                        ...
                        hparams.model_type='prGAN'
                        hparams.model_types=['prGAN']
                        tf.reset_default_graph()
                        estimators = utils.get_estimators(hparams)
                        measurement_losses, l2_losses = utils.load_checkpoints(hparams)
                        estimator = estimators[hparams.model_type] 
                        # hparams.learning_rate = 0.005
                        # hparams.max_update_iter = 5000
                        hparams.mloss2_weight=hparams.mloss2_weight*1.0#1.0/hparams.num_outer_measurements #/np.sqrt(hparams.num_outer_measurements) /np.sqrt(hparams.num_outer_measurements)
                        ...
                        for k in range(hparams.max_outer_iter_step2):            
                            x_est_batch = np.matmul(V_batch,x_main_batch.reshape((x_main_batch.shape[0],x_main_batch.shape[1],1))).reshape((x_main_batch.shape[0],x_main_batch.shape[1]))#/LA.norm(x_main_batch, axis=(1),keepdims=True)
                            # if hparams.method == 'mpr':
                            ...
                            x_main_batch = x_hat_batch                                       
                    # MPRG_step2
                    elif method == 'MPRG_Step2':
                        x_main_batch = x_hat_ini0_batch 
                        ...
                        estimator = estimators['vae']                        
                        for k in range(hparams.max_outer_iter_step1+hparams.max_outer_iter_step2):
                            ...
                            x_main_batch = x_hat_batch
                    # x_hat_refinement_batch=x_hat_batch/LA.norm(x_hat_batch, axis=(1),keepdims=True) 
                    ...
                    # MPRG step 2  
                        ...
                        x_main_batch = x_hat_ini1_batch    
                        for k in range(hparams.max_outer_iter_step2):
                            nv_tilde_batch = np.mean((y_batch_outer-y_batch_mean)*(np.matmul(x_hat_ini1_batch, A_outer)**2), axis=1, keepdims=True)
                            y_tilde_batch  = (y_batch_outer-y_batch_mean)*np.matmul(x_main_batch, A_outer)
                            x_est_batch = x_main_batch -  (1.0/nv_tilde_batch)/hparams.num_outer_measurements *(np.matmul(( nv_tilde_batch*np.matmul(x_main_batch, A_outer)-y_tilde_batch), A_outer.T))          #hparams.outer_learning_rate/hparams.num_outer_measurements       
                        ...
                        for k in range(hparams.max_outer_iter_step2):
                            ...
                            x_est_batch = np.matmul(V_batch,x_main_batch.reshape((x_main_batch.shape[0],x_main_batch.shape[1],1))).reshape((x_main_batch.shape[0],x_main_batch.shape[1]))#/LA.norm(x_main_batch, axis=(1),keepdims=True)
                            # if hparams.method == 'mpr':
                            print('current:',method+'_1','iter:',2*k)    
                            ...
                            print('current:',method+'_2','iter:',2*k+1) 
                            x_est_batch=x_est_batch/LA.norm(x_est_batch, axis=(1),keepdims=True)*LA.norm(x_batch, axis=(1),keepdims=True)
                            z_opt_batch = np.random.randn(hparams.batch_size, 20) #Input to the generator of the GAN
                            x_hat_batch, z_opt_batch = estimator(x_est_batch, z_opt_batch, hparams) # Projectin on the GAN
                            x_hat_batch=x_hat_batch/LA.norm(x_hat_batch, axis=(1),keepdims=True)
                    ...
                    # MPRS
                        ...
                        for k in range(hparams.max_outer_iter_step2):
                            ...
                            print('Current:',method,'iter:',k+hparams.max_outer_iter_step1)
                        ...
                        print(method+' is not in [prGAN,MPRG_Step1,MPRG_Step2,MPRG,MPRGa,MPRS,APPGD]!')
                ...
                if hparams.image_matrix > 0:
                    print('images plot')
                    # outputdir = 'res/mpr_v5_%s_%s_zeta_1overVtilde_50its/'%(hparams.dataset, hparams.nonlinear_model)
                    if  hparams.oblationstudy == 0:
                        if 0:
                            outputdir = 'res/MPR_%s_%s_noit1_%d_noit2_%d/'%(hparams.dataset, hparams.nonlinear_model,hparams.max_outer_iter_step1,hparams.max_outer_iter_step2)
                            ...
                            outputdir = 'res/MPR_%s_%s_relu_xru/'%(hparams.dataset, hparams.nonlinear_model)
                            hparams.savepath=outputdir+'MPR_%s_%s_m_%d_eta_%0.3f_r_%d.png'%(hparams.dataset, hparams.nonlinear_model, hparams.num_outer_measurements,hparams.noise_std,ri)
                    else:
                        outputdir = 'res/MPR_OblationStudy_%s_%s/'%(hparams.dataset, hparams.nonlinear_model)
                        hparams.savepath=outputdir+'MPR_OblationStudy_%s_%s_m_%d_eta_%0.3f_r_%d.png'%(hparams.dataset, hparams.nonlinear_model, hparams.num_outer_measurements,hparams.noise_std,ri)
                    if not os.path.exists('res/'):
                        os.mkdir('res/')
                    if not os.path.exists(outputdir):
                        os.mkdir(outputdir)
                    
                    # hparams.method_ls=['PPower','RefineG','MPRS','APPGD','MPRG']
                    
                    utils.image_matrix_mls(xs_dict, x_hats_dict, view_image, hparams)
                    # hparams.savepath=outputdir+'%s_%s_m_%d_sig_%0.3f_r_%d_step1.png'%(hparams.dataset, hparams.nonlinear_model, hparams.num_outer_measurements,hparams.noise_std,ri)
                    # utils.image_matrix_mls(xs_dict, x_hats_dict, view_image, hparams)
                    img_rec_ls=[]
                    for mi in hparams.method_ls:
                        img_rec_ls = img_rec_ls+[np.stack([vi for vi in x_hats_dict[mi].values()] , axis=0)]
                    # hparams.savepath=outputdir+'%s_%s_m_%d_sig_%0.3f_r_%d_step0.png'%(hparams.dataset, hparams.nonlinear_model, hparams.num_outer_measurements,hparams.noise_std,ri)    
                    np.savez(hparams.savepath[:-4]+'.npz',img_gd=np.stack([vi for vi in xs_dict.values()] , axis=0),img_rec=np.stack(img_rec_ls, axis=0))
                # hparams.method_ls = ['mpr']
...
if __name__ == '__main__':
    ...
    # Pretrained model
    PARSER.add_argument('--pretrained-model-dir', type=str, default='./models/relu', help='Directory containing pretrained model')
    ...
    PARSER.add_argument('--max-outer-iter-step1', type=int, default=20, help='maximum no. of iterations for outer loop Step 1')
    PARSER.add_argument('--max-outer-iter-step2', type=int, default=30, help='maximum no. of iterations for outer loop Step 2')
    # Output
    PARSER.add_argument('--oblationstudy', type=int, default=0, help='oblation study')
