...
def end_to_end(hparams):
    ...
    for i, hidden_size in enumerate(layer_sizes):
        ...
        with tf.compat.v1.variable_scope(layer_name):
            weights = tf.compat.v1.get_variable('weights', shape=[prev_hidden_size, hidden_size],initializer=tf.random_normal_initializer(stddev=1.0))
            biases = tf.compat.v1.get_variable('biases', initializer=tf.zeros([hidden_size]))
            hidden = tf.nn.relu(tf.matmul(hidden, weights) + biases, name=layer_name)
        prev_hidden_size = hidden_size

    with tf.compat.v1.variable_scope('sigmoid_logits'):
        weights = tf.compat.v1.get_variable('weights', shape=[prev_hidden_size, 784],initializer=tf.random_normal_initializer(stddev=1.0))
        biases = tf.compat.v1.get_variable('biases', initializer=tf.zeros([784]))
        logits = tf.add(tf.matmul(hidden, weights), biases, name='logits')

    x_hat_batch = tf.nn.relu(logits)
