...
def main(hparams):
    ...
    # encode
    x_ph = tf.compat.v1.placeholder(tf.float32, [None, hparams.n_input], name='x_ph')
    z_mean, z_log_sigma_sq = model_def.encoder(hparams, x_ph, 'enc', reuse=False)

    # sample
    eps = tf.random.normal((hparams.batch_size, hparams.n_z), 0, 1, dtype=tf.float32)
    ...
    # generator sampler
    z_ph = tf.compat.v1.placeholder(tf.float32, [None, hparams.n_z], name='x_ph')
    ...
    opt = tf.compat.v1.train.AdamOptimizer(learning_rate=hparams.learning_rate)
